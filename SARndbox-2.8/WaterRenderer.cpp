...
// Set the fractal texture to be used in water rendering
void WaterRenderer::setFractalTexture(GLuint fractalTexture) {
    this->fractalTexture = fractalTexture;  // Store the texture ID
}
...
void WaterRenderer::initContext(GLContextData& contextData) const
{
    ...
    for (unsigned int y = 0; y < waterGridSize[1]; ++y)
        for (unsigned int x = 0; x < waterGridSize[0]; ++x, ++vPtr) {
            /* Set the template vertex's position to the pixel center's position: */
...
void WaterRenderer::render(const PTransform& projection, const OGTransform& modelview, GLContextData& contextData) const
{
    ...
    glActiveTextureARB(GL_TEXTURE2_ARB);  // Use texture unit 2 for the fractal texture
    glBindTexture(GL_TEXTURE_2D, fractalTexture);
    glUniform1i(glGetUniformLocationARB(dataItem->waterShader, "fractalTex"), 2);
    ...
    /* Transpose and upload the transposed tangent plane transformation from grid space to eye space: */
